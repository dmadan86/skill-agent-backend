...
export class TraineeAlreadyAssignedError extends AppError {
  constructor(details?: unknown) {
    super(
      "User is already assigned to this training session",
      "TRAINEE_ALREADY_ASSIGNED",
      409,
      details,
    );
  }
...
export class InvalidTrainingStateError extends AppError {
  constructor(
    message: string = "Invalid training state transition",
    details?: unknown,
  ) {
...
export class NoTraineesSpecifiedError extends AppError {
  constructor(details?: unknown) {
    super(
      "No trainees specified for assignment",
      "NO_TRAINEES_SPECIFIED",
      400,
      details,
    );
  }
