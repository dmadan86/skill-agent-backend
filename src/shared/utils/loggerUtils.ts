...
export const apiLogger = {
  ...
  response: (method: string, path: string, statusCode: number, responseTime: number) => {
    if (statusCode >= 400) {
      logger.warn(`API Response: ${method} ${path} - Status: ${statusCode} - Time: ${responseTime}ms`);
    } else {
      logger.debug(`API Response: ${method} ${path} - Status: ${statusCode} - Time: ${responseTime}ms`);
    }
  },
