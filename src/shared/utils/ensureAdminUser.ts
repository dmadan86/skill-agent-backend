...
const ensureAdminUser = async (): Promise<void> => {
  try {
    ...
    if (superadminExists) {
      logger.info("Superadmin user already exists, skipping superadmin creation");
      return;
    }
    ...
    if (!adminEmail || !adminPassword) {
      logger.warn("Superadmin creation skipped: ADMIN_EMAIL or ADMIN_PASSWORD not set in environment variables");
      return;
    }
