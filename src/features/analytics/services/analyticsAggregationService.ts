...
export const aggregateTeamAnalytics = async (teamId: string): Promise<void> => {
  try {
    ...
    const timeRanges = ['last7days', 'last30days', 'last90days', 'ytd'] as const;
    ...
    for (const timeRange of timeRanges) {
      ...
      try {
        ...
        if (aggregation && aggregation.status === 'completed') {
          logger.info(`Aggregation for team ${teamId} already completed, skipping`);
          continue;
        }
        ...
        const performanceTimeline = await analyticsService.getPerformanceTimeline(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const teamMemberProgress = await progressAnalyticsService.getTeamMemberProgress(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const departmentPerformance = await progressAnalyticsService.getDepartmentPerformance(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const trainingCategoryMetrics = await trainingAnalyticsService.getTrainingAgentTypeMetrics(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const scoreDistribution = await evaluationAnalyticsService.getScoreDistribution(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const skillAssessment = await evaluationAnalyticsService.getSkillAssessment(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const assessmentInsights = await evaluationAnalyticsService.getAssessmentInsights(
          teamId,
          timeRange,
          team.owner.toString()
        );
        ...
        const convertedTeamMemberProgress = teamMemberProgress.members.map(member => ({
          ...member,
        ...
        const convertedDepartmentPerformance = departmentPerformance.departments.map(dept => ({
          ...dept,
          departmentId: dept.departmentId ? new mongoose.Types.ObjectId(dept.departmentId) : null
        }));
        aggregation.departmentPerformance = convertedDepartmentPerformance;
        
        aggregation.trainingCategoryMetrics = trainingCategoryMetrics.categories;
        ...
        logger.info(`Successfully aggregated ${timeRange} data for team: ${teamId}`);
      } catch (error) {
        // If an error occurs, update the aggregation record with failed status
        if (aggregation) {
          aggregation.status = 'failed';
          // Log the error details
          logger.error(`Error details for team ${teamId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          await aggregation.save();
        }
        
        logger.error(`Error aggregating ${timeRange} data for team ${teamId}: ${error}`);
      }
    }
...
export const aggregateAllTeamsAnalytics = async (): Promise<void> => {
  try {
    ...
    for (const [batchIndex, batch] of teamBatches.entries()) {
      logger.info(`Processing batch ${batchIndex + 1}/${teamBatches.length} (${batch.length} teams)`);
      
      // Process teams in the batch concurrently
      await Promise.all(
        batch.map(team => 
          aggregateTeamAnalytics(team._id ? team._id.toString() : '')
            .catch(error => {
...
export const getAggregatedAnalytics = async (
  ...
  try {
    ...
    if (!aggregation) {
      logger.debug(`No aggregated analytics found for team ${teamId} for metric ${metric}`);
      return null;
    }
    ...
    switch (metric) {
      ...
      case 'teamMemberProgress':
        result = { members: aggregation.teamMemberProgress.map(member => ({
      ...
      case 'departmentPerformance':
        result = { departments: aggregation.departmentPerformance.map(dept => ({
          departmentId: dept.departmentId ? dept.departmentId.toString() : null,
          name: dept.name,
    ...
    if (!result) {
      logger.debug(`No data found for metric ${metric} in aggregated analytics for team ${teamId}`);
    }
    
    return result;
  } catch (error) {
    logger.error(`Error retrieving aggregated analytics for team ${teamId}, metric ${metric}: ${error}`);
