...
export const calculateMemberTrainingProgress = async (
  ...
  for (const session of trainingSessions) {
    const trainee = session.trainees.find(t => 
...
export const calculateMemberSkillGrowth = async (
  ...
  const currentScore = await calculateMemberEvaluationScore(userId, startDate, endDate);
  
  // Get previous period average score
  const previousScore = await calculateMemberEvaluationScore(userId, prevStartDate, prevEndDate);
...
export const getTeamMemberProgress = async (
  ...
  if (typeof timeRange === 'string') {
    const aggregatedData = await getAggregatedAnalytics(teamId, timeRange, 'teamMemberProgress');
    if (aggregatedData) {
  ...
  for (const memberId of uniqueMemberIds) {
    ...
    const trainingProgress = await calculateMemberTrainingProgress(memberId, startDate, endDate);
    const evaluationScore = await calculateMemberEvaluationScore(memberId, startDate, endDate);
    const skillGrowth = await calculateMemberSkillGrowth(memberId, startDate, endDate);
...
export const getDepartmentPerformance = async (
  ...
  // Check if we have aggregated data available
  if (typeof timeRange === 'string') {
    const aggregatedData = await getAggregatedAnalytics(teamId, timeRange, 'departmentPerformance');
    if (aggregatedData) {
      return aggregatedData;
    }
  }
  ...
  for (const user of users) {
    ...
    const departmentName = user.department && typeof user.department === 'object' && 'name' in user.department
  ...
  for (const [departmentId, data] of departmentMap.entries()) {
    ...
    for (const memberId of data.members) {
      const evalScore = await calculateMemberEvaluationScore(memberId, startDate, endDate);
      const completionRate = await calculateMemberCompletionRate(memberId, startDate, endDate);
    ...
    const completionRate = data.completionRates.length > 0 
      ? parseFloat((data.completionRates.reduce((a, b) => a + b, 0) / data.completionRates.length).toFixed(1)) 
      : 0;
...
export const calculateUserTrainingProgress = async (
  ...
  trainingSessions.forEach(session => {
    const userTrainee = session.trainees.find(trainee => 
      trainee.userId.toString() === userId
    );
    
    if (userTrainee) {
      totalAssigned++;
      
      if (userTrainee.status === 'Completed' && 
  ...
  return totalAssigned > 0 ? parseFloat(((totalCompleted / totalAssigned) * 100).toFixed(1)) : 0;
};
...
export const calculateDepartmentAvgScore = async (
  ...
  return scores.length > 0 ? parseFloat(scores[0].departmentAvgScore.toFixed(1)) : 0;
};
