...
export const verifyTeamAccess = async (
  ...
  if (requireOwnership && team.owner.toString() !== userId) {
    throw new ForbiddenError(
      "You do not have permission to access team analytics",
    );
  }
...
export const getOverviewMetrics = async (
  ...
  if (typeof timeRange === "string") {
    ...
    if (aggregatedData) {
      return {
        ...
        trends: {
          avgTrainingScore: {
            value: aggregatedData.avgTrainingScore,
            change: 0,
          },
          trainingCompletionRate: {
            value: aggregatedData.trainingCompletionRate,
            change: 0,
          },
          avgTimeToProficiency: {
            value: aggregatedData.avgTimeToProficiency,
            change: 0,
            unit: "days",
          },
          activeTrainingSessions: {
            value: aggregatedData.activeTrainingSessions,
            change: 0,
          },
        },
      };
  ...
  const avgTrainingScore = await calculateAverageTrainingScore(
    teamId,
    startDate,
    endDate,
  );
  const trainingCompletionRate = await calculateCompletionRate(
    teamId,
    startDate,
    endDate,
  );
  const avgTimeToProficiency = await calculateAverageTimeToProficiency(
    teamId,
    startDate,
    endDate,
  );
  ...
  const prevAvgTrainingScore = await calculateAverageTrainingScore(
    teamId,
    prevStartDate,
    prevEndDate,
  );
  const prevTrainingCompletionRate = await calculateCompletionRate(
    teamId,
    prevStartDate,
    prevEndDate,
  );
  const prevAvgTimeToProficiency = await calculateAverageTimeToProficiency(
    teamId,
    prevStartDate,
    prevEndDate,
  );
  const prevActiveTrainingSessions = await countActiveTrainingSessions(
    teamId,
    prevEndDate,
  );

  // Calculate percentage changes
  const avgTrainingScoreChange = calculatePercentageChange(
    avgTrainingScore,
    prevAvgTrainingScore,
  );
  const trainingCompletionRateChange = calculatePercentageChange(
    trainingCompletionRate,
    prevTrainingCompletionRate,
  );
  const avgTimeToProficiencyChange = calculateDayChange(
    avgTimeToProficiency,
    prevAvgTimeToProficiency,
  );
  const activeTrainingSessionsChange =
    activeTrainingSessions - prevActiveTrainingSessions;
  ...
  return {
    ...
    trends: {
      avgTrainingScore: {
        value: avgTrainingScore,
        change: avgTrainingScoreChange,
      },
      trainingCompletionRate: {
        value: trainingCompletionRate,
        change: trainingCompletionRateChange,
      },
      avgTimeToProficiency: {
        value: avgTimeToProficiency,
        change: avgTimeToProficiencyChange,
        unit: "days",
      },
      activeTrainingSessions: {
        value: activeTrainingSessions,
        change: activeTrainingSessionsChange,
      },
    },
  };
};
...
export const calculateAverageTrainingScore = async (
  ...
  return progressRecords.length > 0
    ? parseFloat(progressRecords[0].teamAvgScore.toFixed(1))
    : 0;
};
...
export const calculateUserTimeToProficiency = async (
  ...
  return Math.ceil(
    (proficiencyDate.getTime() - assignedDate.getTime()) /
      (1000 * 60 * 60 * 24),
  );
};
...
export const calculateAverageEvaluationScore = async (
  ...
  return progressRecords.length > 0
    ? parseFloat(progressRecords[0].avgScore.toFixed(1))
    : 0;
};
...
export const calculateAverageProficiencyLevel = async (
  ...
  const avgTrainingScore =
    trainingScores.length > 0 ? trainingScores[0].avgTrainingScore : 0;
  const avgEvalScore =
    evaluationScores.length > 0 ? evaluationScores[0].avgScore : 0;
