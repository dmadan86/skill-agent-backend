...
export const getTeamById = async (teamId: string, userId: string): Promise<ITeam> => {
  ...
  const isMember = team.members.some(member => {
    ...
    return member._id.toString() === userId || team.owner._id.toString() === userId
  });
...
export const inviteToTeam = async (
  ...
  for (const email of normalizedEmails) {
    if (emailsWithPendingInvites.has(email)) {
      logger.info("An invitation has already been sent to this email: "+email);
      results.failure.push({
        email,
    ...
    if (team.members.some(member => member.toString() === existingUser._id.toString())) {
    ...
    if (existingUser.managedBy && inviter.managedBy && 
    ...
    try {
      ...
      throw new AppError('Failed to create secure invitation', 'INVITE_GENERATION_FAILED', 500);
    }
...
export const acceptInvite = async (token: string): Promise<{ team: ITeam; userId?: string }> => {
...
export const getTeamInvites = async (teamId: string, requesterId: string, query: any): Promise<ITeamInvite[]> => {
  const team = await Team.findById(teamId);
