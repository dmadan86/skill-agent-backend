...
async function processRecords(
  ...
  for (const data of userData) {
    ...
    if (existingUser) {
      ...
      const isMember = team.members.some(
        (m: any) => m.toString() === existingUser._id.toString(),
      );
  ...
  if (userIdsToAdd.length > 0) {
    try {
      const addResult = await teamService.addTeamMembers(
        teamId,
        userIdsToAdd,
        requesterId,
      );
      ...
      for (const success of addResult.results.success) {
        const user = existingUsers.find(
          (u) => u._id.toString() === success.userId,
        );
        if (user) {
      ...
      for (const failure of addResult.results.failure) {
        const user = existingUsers.find(
          (u) => u._id.toString() === failure.userId,
        );
        if (user) {
          results.failure.push({
            email: user.email,
  ...
  if (emailsToInvite.length > 0) {
    try {
      const inviteResult = await teamService.inviteToTeam(
        teamId,
        emailsToInvite,
        requesterId,
      );
