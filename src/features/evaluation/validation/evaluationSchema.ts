...
const skillAssessmentSchema = z.object({
  ...
  }).min(0, 'Score must be at least 0').max(100, 'Score cannot exceed 100'),
  weight: z.number().min(1, 'Weight must be at least 1').max(10, 'Weight cannot exceed 10').default(1),
});
...
const createEvaluationSchema = z.object({
  ...
  }).min(3, 'Title must be at least 3 characters').max(100),
...
export type UpdateEvaluationProgressInput = z.infer<typeof updateEvaluationProgressSchema>;
...
export type AddImprovementAreasInput = z.infer<typeof addImprovementAreasSchema>;
